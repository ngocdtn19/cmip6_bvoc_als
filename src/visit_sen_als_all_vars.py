...
class VisitSenAls:
    ...
    def load_data(self):
        ...
        tas[self.var_name] = (
            self.ds_vars["co2_met_luc"][self.var_name]
            - self.ds_vars["co2_luc_tas"][self.var_name]
        )
        rsds[self.var_name] = (
            self.ds_vars["co2_met_luc"][self.var_name]
            - self.ds_vars["co2_luc_rsds"][self.var_name]
        )
        pr[self.var_name] = (
            self.ds_vars["co2_met_luc"][self.var_name]
            - self.ds_vars["co2_luc_pr"][self.var_name]
        )
    ...
    def cal_drivers_pixel_level(self, ds_pixel):
        ...
        stacked = np.stack((tas * met, rsds * met, pr * met), axis=-1)
    ...
    def plt_glob_rate_drivers(self):
        ...
        for i, (df, tit) in enumerate(zip([self.driver_glob_cao], ["cao"])):
            ax = axes[i]
            sns.barplot(
                df,
                x="driver",
    ...
    def plt_glob_driver_pixel(self, mode="cao"):
        ...
        cmap = matplotlib.colors.ListedColormap(
            matplotlib.colormaps["Set3"].colors[3:6]
        )
        center = [0.5, 1.5, 2.5]
        ...
        cbar.ax.set_xticklabels(["tas", "rsds", "pr"], size=14)
